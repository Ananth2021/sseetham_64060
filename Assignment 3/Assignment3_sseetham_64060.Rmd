...
train.df <- NB3[train.index, ] # 
valid.df <- NB3[valid.index, ]
train <- NB3[train.index, ]
valid = NB3[train.index,]
```
...
LoanCC1 <- 77/3000  # 77 is the value for Loan and CC =1 as per pivot table.  and 300 is the total count of 1924 + 198 + 801 + 77 = 3000 
LoanCC1 # which is 26 %.
```
...
melt2 = melt(train,id=c("CreditCard"),variable = "Online") # Melting Credicatd data with reference to online
...
LoanCC # Indicates Credit Card count in reference with onlin.
```
...
library(caret)
library(e1071)
naive.train = train.df[,c(10,13,14)] # training data is from Personal loan, Creditcard and online. column 
naive.test =valid.df[,c(10,13,14)] # testing set data from the same cloumns of data 
naivebayes = naiveBayes(Personal.Loan~.,data=naive.train) # applying naive bayes to personal loan and training data. 
naivebayes
